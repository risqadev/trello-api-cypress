...
describe('Trello API Challenge', () => {
  context('Cards context', () => {
    ...
    it('Should edit a card', () => {
      cy.editCard(fixData.cardToUpdate.id, {
        name: fixData.cardToUpdate.editedName,
        desc: fixData.cardToUpdate.editedDesc,
        idList: fixData.fixedList.doing.id
      }).should(({status, body}) => {
        expect(status).to.eq(200)
        expect(body).to.have.property('id').to.eq(fixData.cardToUpdate.id)
        expect(body).to.have.property('name').to.eq(fixData.cardToUpdate.editedName)
        expect(body).to.have.property('desc').to.eq(fixData.cardToUpdate.editedDesc)
        expect(body).to.have.property('idBoard').to.eq(fixData.fixedBoard.id)
      ...
      cy.get('@card edit')
        .then(() => {
          cy.request('GET',
            `/1/cards/${fixData.cardToUpdate.id}/?key=${appKey}&token=${token}`
          ).as('get card')
            .should(({status, body}) => {
              expect(status).to.eq(200)
              expect(body).to.have.property('id').to.eq(fixData.cardToUpdate.id)
      ...
      cy.get('@get card')
        .then(() => {
          cy.editCard(fixData.cardToUpdate.id, {
            name: fixData.cardToUpdate.name,
            desc: fixData.cardToUpdate.desc,
            idList: fixData.fixedList.todo.id
          })
        }).as('reset card')
    })
